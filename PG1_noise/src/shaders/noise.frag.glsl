precision highp float;  // needed for: indexing an array, const array, modulo %
...
float perlin_noise_1d(float x) {
	...
	*/
	...
	float c_i = floor(x);
	float c_i_next = c_i + 1.;

	float g_i = gradients(hash_func(vec2(c_i, 0.))).x;
	float g_i_next = gradients(hash_func(vec2(c_i_next, 0.))).x;

	float phi_i = g_i * (x - float(c_i));
	float phi_i_next = g_i_next * (x - float(c_i_next));

	float t = x - float(c_i);
	float alpha = blending_weight_poly(t);
	float result = mix(phi_i, phi_i_next, alpha);
	return result;
}
...
float perlin_fbm_1d(float x) {
	...
	*/
	...
	for (int i = 0; i < num_octaves; i++) {
		float arg = x * pow(w1, float(i));
		result += pow(A1, float(i)) * perlin_noise_1d(arg);
	}
...
float perlin_noise(vec2 point) {
	...
	*/
	...
	float x = floor(point.x);
	float y = floor(point.y);

	vec2 c_00 = vec2(x, y);
	vec2 c_01 = vec2(x, y + 1.);
	vec2 c_10 = vec2(x + 1., y);
	vec2 c_11 = vec2(x + 1., y + 1.);
	...
	float s = dot(g_00, point - c_00);
	float u = dot(g_01, point - c_01);
	float t = dot(g_10, point - c_10);
	float v = dot(g_11, point - c_11);

	float t_x = point.x - x;
	float t_y = point.y - y;

	float f_x = blending_weight_poly(t_x);
	float f_y = blending_weight_poly(t_y);
...
float perlin_fbm(vec2 point) {
	...
	*/
	...
	float result = 0.;
	for (int i = 0; i < num_octaves; i++) {
		vec2 arg = point * pow(w1, float(i));
		result += pow(A1, float(i)) * perlin_noise(arg);
	}
...
float turbulence(vec2 point) {
	...
	*/
	...
	for (int i = 0; i < num_octaves; i++) {
		vec2 arg = point * pow(w1, float(i));
		result += pow(A1, float(i)) * abs(perlin_noise(arg));
	}
