...
vec3 lighting(
	...
	*/
	...
	//diffuse
	vec3 diffuse_coeff = vec3(0.);
	vec3 l = normalize(light.position - object_point);
	float l_dot_n = dot(l, n);
	
	diffuse_coeff = mat.diffuse * mat.color;
	...
	*/
	...
	vec3 spec_coeff = vec3(0.);
	vec3 r = reflect(-l, n);
	float r_dot_v = dot(r, v);
	
	spec_coeff = mat.specular * mat.color;
	...
	float col_distance_shadow = 0.;
	vec3 col_normal_shadow = vec3(0.);
	int material_id_shadow = 0;
	
	vec3 object_point_shadow = object_point + 0.001 * n;
	float light_object_distance = length(object_point_shadow - light.position);
	float shadow = 1.;
	
    if (ray_intersection(object_point_shadow, l, col_distance_shadow, col_normal_shadow, material_id_shadow)) {
		if (col_distance_shadow < light_object_distance) {
			shadow = 0.;
		}
    }

	float s = mat.shininess;
	return shadow * light.color * ((l_dot_n * diffuse_coeff) + pow(max(dot_spec, 0.), s) * spec_coeff);
}
...
vec3 render_light(vec3 ray_origin, vec3 ray_direction) {
	...
	*/
	...
	/*vec3 reflect_color          = pix_color;
	float reflection_weight = 0.;
	float product_reflect_weights = 1.;
